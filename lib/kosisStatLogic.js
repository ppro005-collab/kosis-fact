...
export const statLogic = {
  ...
  getYouthEducationSystem: (row) => {
    ...
    if (g === 3) return '\uc804\ubb38\ub300\uc878 (3\ub144\uc81c \ud3ec\ud568)';
    if (g === 4) return '\ub300\ud559\uad50\uc878 (4\ub144\uc81c)';
    if (g === 5) return '\ub300\ud559\uc6d0\uc878 \uc774\uc0c1';
    return '\uae30\ud0c0';
  },

  /** 피벗용 연령대 (만연령) */
  getAgeBandYouth: (row) => {
    const a = parseInt(row['\ub9cc\uc5f0\ub839'] || 0, 10);
    if (a >= 15 && a <= 19) return '10\ub300';
    if (a >= 20 && a <= 29) return '20\ub300';
    if (a >= 30) return '30\ub300 \uc774\uc0c1';
    return '\uacc4';
  }
};
...
export const DIMENSION_LABEL_EXTRACTORS = {
  ...
  '\uacbd\uc81c\ud65c\ub3d9\uc0c1\ud0dc': (row) => statLogic.getEconomicStatus(row),
  ...
  '\ucde8\uc5c5\uacbd\ud5d8': (row) => statLogic.getJobExperienceCount(row),
